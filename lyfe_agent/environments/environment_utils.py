"""
utils for the environment settings
"""
import warnings
import numpy as np
...
def world_time_update(world_time, sim_speed):
    """Update the world time"""
    return world_time + timedelta(seconds=sim_speed)


def world_time_format_str(world_time):
    """Format the world time list into a string"""
    return world_time.strftime("%m/%d %H:%M")
...
def ascii_to_string(observation):
    ...
    end_of_string_index = (
        end_of_string_index[0] if end_of_string_index.size > 0 else len(observation)
    )
