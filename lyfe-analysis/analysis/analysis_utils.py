...
# Depracated
def log_to_usage(filename):
    """Convert log to pandas dataframe for LLM usage."""
    # Read the log file
    with open(filename, "r") as file:
        log_data = file.readlines()

    # Initialize lists to store extracted completions and prompts
    completions = []
    prompts = []

    # Define a regex pattern to match the completion and prompt numbers
    pattern = re.compile(r"\[COMPLETION\](\d+)\[PROMPT\](\d+)")

    # Iterate through each line of the log data
    for line in log_data:
        # Find matches in the line using the regex pattern
        match = pattern.search(line)

        # If a match is found, append the completion and prompt numbers to their respective lists
        if match:
            completions.append(int(match.group(1)))
            prompts.append(int(match.group(2)))

    # Create a pandas dataframe with the extracted data
    data = {"Completions": completions, "Prompts": prompts}
...
def generate_dialogues_json(df, min_length=1):
    ...
    for agent1, agent2 in combinations(agents, 2):
        ...
        df_dialogue = df[
            ((df["sender"] == agent1) & (df["receiver"] == agent2))
...
def _format_override(hydra_path):
    ...
    with open(input_yaml_file_path, "r") as input_file, open(
        output_yaml_file_path, "w"
    ) as output_file:
